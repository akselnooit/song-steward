...
- **Globalny filtr lokalizacji** — Stały filtr (np. w nagłówku lub ustawieniach) pozwalający wybrać aktywną lokalizację (Wrocław, Ustroń, Brunstad, Ukraina). Wpływa na całą aplikację: dashboard, wyszukiwarka, historia śpiewania — wszystko filtrowane pod kątem wybranej lokalizacji.

## Nabożeństwa

- **Filtry nad liderami w kafelku dashboardu** — W widoku filtrów (np. NeverSungSection), filtry tagów powinny być wyświetlane powyżej listy liderów uwielbienia, nie poniżej.
...
- **Zaplanuj na najbliższe nabożeństwo z podglądu starszego** — Gdy przeglądamy stare nabożeństwo i otworzymy tam pieśń, chcemy mieć możliwość szybkiego zaplanowania jej na najbliższe (przyszłe) nabożeństwo — bez wychodzenia z podglądu.
...
- **Typy nabożeństw: rozdzielenie lokalizacji i typu** — Obecnie typ nabożeństwa łączy lokalizację i typ w jednym polu (np. "Wrocław - Ogólne"). Rozdzielić na dwa osobne wymiary: `location` (Wrocław, Ustroń, Brunstad, Ukraina) i `service_type` (Ogólne, Środowe, Młodzieżowe). Odpowiednio zaktualizować widok ustawień (zakładka "Typy nabożeństw") — dwa osobne ekrany lub sekcje do zarządzania lokalizacjami i typami.
