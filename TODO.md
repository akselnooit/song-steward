...
- **Tag "Uświęcenie" — dodanie i otagowanie pieśni** — Dodać tag "Uświęcenie" (kategoria do ustalenia — najpewniej 📖 Temat), a następnie przejść przez pieśni i zaznaczyć te, które mówią o uświęceniu. Cel: jednym filtrem dostać zestaw pieśni na nabożeństwo o tej tematyce.
- **Duplikat tagów: "Ponowne przyjście" vs "Przyjście Jezusa"** — Sprawdzić, czy te dwa tagi nie znaczą tego samego. Porównać, ile pieśni ma każdy z nich i które pieśni mają oba naraz — duży przekrój to mocny sygnał, że to jeden tag w dwóch nazwach. Jeśli tak: wybrać nazwę, przepiąć pieśni z odrzuconego tagu i go usunąć. Uwaga: jeśli różnica jest realna (powrót Chrystusa w chwale vs. pierwsze przyjście / narodzenie), to nie duplikat — wtedy dopisać obu tagom `description`, żeby było jasne, kiedy którego użyć.
...
- **Filtr statystyk „bez prowadzącego"** — Prowadzący muzykę jest polem opcjonalnym, więc istnieją nabożeństwa z `worship_leader_id = NULL`. Filtr prowadzącego w Ustawieniach ma dziś „Wszyscy" + listę osób, ale nie da się wybrać „tylko nabożeństwa bez prowadzącego". Wymaga nowego trybu w `get_top_sung` i `get_never_sung` — obecnie `p_leader_id = NULL` znaczy „nie filtruj", więc nie da się nim wyrazić „tylko NULL" (np. dodatkowy parametr `p_leader_unassigned BOOLEAN`). Statystyki bez filtra prowadzącego już teraz liczą te nabożeństwa poprawnie.
